...
static void render_callback(Canvas *canvas, void *ctx)
{
    ...
    canvas_draw_str_aligned(canvas, 64, 7, AlignCenter, AlignCenter, "Credits");
...
void save_count(uint32_t count)
{
    ...
    do
    {
        if (storage_file_exists(storage, APP_DATA_PATH("credit_counter_storage.txt")))
        {
            storage_simply_remove(storage, APP_DATA_PATH("credit_counter_storage.txt"));
        }

        if (!flipper_format_file_open_new(file, APP_DATA_PATH("credit_counter_storage.txt")))
            break;
        if (!flipper_format_write_uint32(file, "UINT", &count, 1))
            break;
    } while (0);
...
uint32_t read_count()
{
    ...
    do
    {
        if (!flipper_format_file_open_existing(file, APP_DATA_PATH("credit_counter_storage.txt")))
            break;
        if (!flipper_format_read_uint32(file, "UINT", &uint32_value, 1))
            break;
    } while (0);
